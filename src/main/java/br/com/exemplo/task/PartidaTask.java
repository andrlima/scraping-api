...
import br.com.exemplo.services.ScrapingServices;
import br.com.exemplo.util.DataUtil;
...
public class PartidaTask {
	...
	//Faz o bot executar a cada 30 segundos (Pesquisar pelo no CRON para spring)
	...
	@Scheduled(cron = "0/30 * 16-23 * * SAN", zone = TIME_ZONE)
	public void taskPartidaDomingoTarde() {
		inicializaAgendamento("taskPartidaDomingoTarde()");
		
	}
	
	@Scheduled(cron = "0/30 * 11-13 * * SAN", zone = TIME_ZONE)
	...
	private void inicializaAgendamento(String diaSemana) {
		this.gravaLogInfo(String.format("%s: %s", diaSemana, DataUtil.formatarDataEmString(new Date(), DD_MM_YYYY_HH_MM_SS )));
